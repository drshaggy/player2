...
- **DYNAMIC_FEN_PLAN.md**
- **ENGINE_SAAS_PLAN.md** - Plan (not yet implemented) for moving Stockfish off the public chess-api.com WebSocket onto a self-hosted worker on percy.network, plus free/paid SaaS gating. Covers engine-worker/, /api/engine, /api/games, plan column on profiles, and RLS backstop.
...
- **Dangerous (flag for human review before changing):**
  ...
  - `ENGINE_URL`, `ENGINE_KEY`, `ENGINE_FALLBACK_WS` — self-hosted Stockfish gateway (see ENGINE_SAAS_PLAN.md). Misconfig breaks `/api/engine`; fallback WS only when `ENGINE_URL` is unset.
  - RLS policies on `games` — the planned free-tier INSERT gate (`free_tier_one_game_max`) is a billing backstop; do not modify without coordinating with `/api/games` server-side check. Source of truth stays `supabase/migrations/20260629000103_rls_policies.sql`.
...
- [x] Fix game restore from persisted FEN — `restoreGame` was calling `chessGame.reset()` + replay SANs, which failed for consultation-FEN games (moves illegal from standard position). Now loads `game.current_fen` directly.
- [x] Fix SET_FEN regex capturing only first FEN field — `[^\s\n\r]+` stopped at first space, leaving `w KQkq - 0 1` leaking into chat. Now matches all 6 FEN fields.
- [x] Show "Coach is thinking..." during AI move — `makeAIMove` toggles `isTyping` via shared ref (same pattern as `setChatMessagesRef`), not just during consultation chat.
- [x] FEN-based opening lookups for consultation games — when history is empty (game started from consultation FEN), `getOpeningMovesByFen` traverses the opening book tree matching piece placement + side to move, and `getMasterOpeningMovesByFen` uses Lichess's FEN endpoint. Without this, the LLM got no opening theory context for Caro-Kann etc.
- [x] Remove Cerebras default — `LLM_ENDPOINT` and `LLM_MODEL` are now required (no silent fallback to `api.cerebras.ai` / `gemma-4-31b`). App throws a clear error if unset.
- [x] Consultation opening → most-popular line — when the consultation message mentions a known opening (e.g. "Play caro kann from middle game"), `src/lib/consultation/openingLineFinder.ts` detects it against the hand-authored book (full name + stripped "head" fallback so partial mentions match) and queries the Lichess masters explorer from the spine tip, following the top move by game frequency until the line peters out (max 14 plies / <20 games). It builds a **menu** of White-to-move positions at increasing depth (start of opening, then each Black reply deeper), each carrying the SAN line from the start position, and injects it into the consultation prompt. The **LLM decides** which depth matches the player's intent ("play caro kann" = start, "caro kann middle game" = deep) and emits the chosen FEN verbatim as `SET_FEN`. On `TRANSITION_TO_GAME` the route validates the LLM's FEN against the menu (falls back to the start position if the LLM hallucinated) and passes the chosen position's SAN line (`suggestedLine`) to the client; `useCoachChat` replays those moves from the start position instead of bare `load(fen)` so chess.js history is populated (move history panel, opening book lookups, and move numbering all depend on it). No opening detected / Lichess down → the LLM-driven `SET_FEN` path is unchanged (bare `load(fen)`).
- [x] Fix `makeAIMove` error handling — `undo()` was firing on every error, reverting the **player's** last move when the AI never moved (Stockfish WebSocket timeout, LLM 500). Now `undo()` only runs if the AI move was actually applied (`aiMoveApplied` flag). Pre-move `network`-category errors (e.g. WebSocket timeout) auto-retry once after 2s (`isRetry` guard) so the player isn't left stuck with their move on the board and no way to trigger the AI.
- [x] Stockfish WebSocket fallback — when `wss://chess-api.com/v1` times out or errors, the WebSocket failure is caught by an inner try/catch (does not propagate to the outer error handler). Fallback candidates are built from opening-book moves (or legal moves as a last resort) so the pipeline still gets candidate moves and the game continues without engine eval. The pipeline merges these with Lichess master moves and book context, so the LLM can still pick a reasonable move. UCI history is now sent to `/api/move` so the pipeline can use the non-auth-gated Lichess play-parameter endpoint for master game lookups.
- [ ] **ENGINE_SAAS_PLAN** — self-host Stockfish on percy.network (`engine-worker/`), Vercel proxy at `/api/engine`, free/paid gating via `profiles.plan` (default `free`), 1-game-per-account lifetime limit enforced at `/api/games` (A) + `/api/chat` pre-check (B) + RLS INSERT backstop (C). No Redis — in-process FEN cache on worker, in-memory LLM burst limiter on Vercel. See `ENGINE_SAAS_PLAN.md`.
